...
public class Registration implements RegistrationDAO {
	public static List<Movie> moviearray = new ArrayList<Movie>();
	@Autowired
	...
	public Userlogin insertlogin(Userlogin userlogin) {
		...
		try {
			...
			if (rs.next()) {
				if (rs.getString("username").equals(userlogin.getUsername())
						&& (rs.getString("password").equals(userlogin
								.getPassword()))) {

					System.out.println("yes");
					userlogin.setCheck(true);
				} else {
					userlogin.setCheck(false);
	...
	public Movie insertmovie(Movie movie) {
		...
		try {
			...
			while (rs.next()) {
				Movie movies = new Movie(rs.getString("movie_name"),
